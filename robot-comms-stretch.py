# =============================================================================
#  robot-comms-stretch.py  —  Stretch Hello Robot 3 inter-robot communication node
# =============================================================================
#
#  RUNNING
#  -------
...
#    source ~/ament_ws/install/setup.bash       # your workspace, if needed
#    python3 robot-comms-stretch.py
#
#  To override the namespace at runtime without editing the file:
#
#    python3 robot-comms-stretch.py --ros-args -r __ns:=/stretch1
#
#  CONFIGURATION
#  -------------
...
#    NAMESPACE    — ROS2 topic namespace, e.g. "/stretch1"  ("" to disable)
#    PORT         — HTTP port this robot's API listens on
#    STATIC_PEERS — optional dict of known peer IPs to poll unconditionally
#
#  NOTES: Stretch-specific
#  -----------------------
#  - /battery percentage is NaN on Stretch 3; voltage and current are used instead.
#    Battery percentage is estimated from voltage (12V lead-acid: 11.8V=0%, 12.7V=100%).
#  - /is_runstopped (std_msgs/Bool) is exposed so peers know if the robot is stopped.
#  - No dock status — Stretch does not have a docking station.
#
#  DEPENDENCIES
#  ------------
...
# ─────────────────────────────────────────
ROBOT_ID        = "stretch1"
NAMESPACE       = ""     # Stretch doesn't use namespaces by default — set if needed
PORT            = 8000
...
def estimate_battery_pct(voltage: float) -> float:
    """Estimate battery % from voltage (linear, 12V lead-acid)."""
    pct = (voltage - BATTERY_V_MIN) / (BATTERY_V_MAX - BATTERY_V_MIN) * 100.0
...
own_state: dict = {
    ...
    "battery_current":    None,   # amps (raw); positive = discharging
    "battery_percentage": None,   # estimated from voltage
    "is_runstopped":      None,   # True = runstopped (motors disabled)
    ...
    "heartbeat_ts":       None,   # Unix timestamp (float), ticks every POLL_INTERVAL
                                  # regardless of ROS activity — use to confirm process alive
}
...
class RobotStateListener(Node):
    def __init__(self):
        ...
        # Stretch typically publishes on global topics (no namespace), so NAMESPACE
        # defaults to "" — leave blank unless running a multi-robot setup.
        super().__init__("coords_server", namespace=NAMESPACE or None)

        self.create_subscription(
            Odometry,
            "odom",             # relative — resolves to <namespace>/odom or /odom
            self.odom_callback,
            10
        )
        self.create_subscription(
            BatteryState,
            "battery",          # /battery on Stretch (not /battery_state)
            self.battery_callback,
            10
        )
        self.create_subscription(
            Bool,
            "is_runstopped",    # True when e-stop / runstop is engaged
            self.runstopped_callback,
            10
        )
    ...
    def battery_callback(self, msg: BatteryState):
        voltage = msg.voltage
        own_state["battery_voltage"]    = round(voltage, 3)
        own_state["battery_current"]    = round(msg.current, 3)
        # percentage is NaN on Stretch 3 — estimate from voltage instead
        own_state["battery_percentage"] = estimate_battery_pct(voltage)
        own_state["last_updated"]       = now_iso()

    def runstopped_callback(self, msg: Bool):
        own_state["is_runstopped"] = msg.data
        own_state["last_updated"]  = now_iso()
...
def fmt_bool(val, true_str: str, false_str: str, true_color: str = "green", false_color: str = "yellow") -> str:
    if val is None:
        return "[dim]N/A[/dim]"
...
def fmt_battery(pct, voltage) -> str:
    if pct is None and voltage is None:
        return "[dim]N/A[/dim]"
    color = "green" if (pct or 0) >= 60 else ("yellow" if (pct or 0) >= 25 else "red")
    pct_str = f"{pct:.1f}%" if pct is not None else "?%"
    v_str   = f"{voltage:.2f}V" if voltage is not None else ""
    return f"[{color}]{pct_str}[/{color}] [dim]{v_str}[/dim]"
...
def build_own_panel() -> Panel:
    ...
    table.add_row("Battery",      fmt_battery(s.get("battery_percentage"), s.get("battery_voltage")))
    table.add_row("Current",      fmt_float(s.get("battery_current"), 2) + " A" if s.get("battery_current") is not None else "[dim]N/A[/dim]")
    table.add_row("Runstopped",   fmt_bool(s.get("is_runstopped"), "YES", "No", true_color="red", false_color="green"))
    ...
    return Panel(table, title="[bold green]● This Robot (Stretch)[/bold green]", border_style="green")
...
def build_peers_table() -> Table:
    ...
    table.add_column("Runstopped",  min_width=10)
    ...
    for robot_id in sorted(all_ids):
        ...
        # Use heartbeat_ts from state if available for accurate status
        hb_ts = s.get("heartbeat_ts")
        if hb_ts is not None:
            hb_age = time.time() - hb_ts
            if hb_age <= POLL_INTERVAL * 3:
                status = "[green]● Online[/green]"
            elif hb_age <= HEARTBEAT_TTL:
        ...
        table.add_row(
            ...
            fmt_bool(s.get("is_runstopped"), "YES", "No", true_color="red", false_color="green"),
