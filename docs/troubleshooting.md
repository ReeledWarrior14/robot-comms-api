...
## `last_updated` is very old - ROS callbacks stopped updating

The server was receiving ROS messages but they stopped. `last_updated` reflects the last ROS callback time, not the last HTTP/API request.
...
Note: `heartbeat_ts` will still be current even if `last_updated` is stale - they are updated by different threads. If `last_api_query` is current at the same time, that means the API is still being queried even though ROS data is stale. If `heartbeat_ts` is also stale, the Python process itself is frozen or dead.
